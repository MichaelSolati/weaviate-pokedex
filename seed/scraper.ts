...
(async () => {
  ...
  do {
    ...
    // Get Pokemon's main picture
    const image = "https:" + $('a[href*="/wiki/File:"] > img').attr("src");
    samples.push(image);

    // Get Pokemon's other pictures
    const spritesTable = $("h3 > #Sprites").parent().next();
    const spriteImages = spritesTable.find('img').toArray();

    for (let sprite of spriteImages) {
      const spriteImage = "https:" + sprite.attribs["src"];
      samples.push(spriteImage);
    }
  ...
  writeFileSync("./seed/pokedex.json", JSON.stringify(pokedex, null, "\t"));
})();
